...
/**
 * Class {@code OpenLimTariffsPage} is an implementation of {@code Command} for
 * opening page with all limited tariffs.
 * 
 * @author Denis Bohnat
 * @version 1.0
 * @see Command
 */
public class OpenLimTariffsPage implements Command {

	private static final Logger logger = LogManager.getLogger(OpenLimTariffsPage.class.getName());

	/**
	 * Performs the command that gets list of all limited tariffs entities from
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			request.getRequestDispatcher(JSPNames.LIM_TARIFFS_PAGE).forward(request, response);
